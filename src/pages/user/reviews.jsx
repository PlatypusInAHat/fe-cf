...
export default function MyReviewsPage() {
  ...
  const filteredReviews = reviews
    .filter((r) => searchCafe === "" || r.cafeName.toLowerCase().includes(searchCafe.toLowerCase()))
    .filter((r) => filterRating === "" || r.rating === Number.parseInt(filterRating))
    .sort((a, b) => {
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <Card className="p-6 mb-6 bg-card">
          <div className="grid grid-cols-1 md) => setSearchCafe(e.target.value)}
          ...
          {filteredReviews.length === 0 ? (
            <Card className="p-8 text-center bg-card">
              <p className="text-muted-foreground">
                {searchCafe || filterRating ? "No reviews match your filters." : "No reviews yet."}
              </p>
            </Card>
          ) : (
            filteredReviews.map((review) => (
              <Card key={review.id} className="p-6 bg-card hover) => handleDelete(review.id)}
                    >
